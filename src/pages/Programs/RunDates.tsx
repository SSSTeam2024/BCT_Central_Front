...
const RunDates: React.FC<RunDatesProps> = ({ setActiveTab }) => {
  ...
  const saveProgress = async (tabNumber: string, data: typeof programmData) => {
    ...
    if (
      runDatesLocation?.program?.company_id! === null ||
      runDatesLocation?.program?.company_id! === ""
    ) {
      data["programDetails"]["school_id"] =
        runDatesLocation?.program?.school_id!;
    }
    if (
      runDatesLocation?.program?.school_id! === null ||
      runDatesLocation?.program?.school_id! === ""
    ) {
    ...
    data["programDetails"]["_id"] = localStorage.getItem("prog_id")!;

    await updateProgram({
      id: localStorage.getItem("prog_id")!,
      updatedProgram: data,
    });
    setActiveTab(3);
  };
